#!/bin/bash
# LoRA SFT sweep on commonsense-170k for a BHyT-pretrained Llama-3.2-1B.
# Usage: bash scripts/bhyt/com170k/sft_lora_sweep_1b.sh GPU_ID {bhyt|bhytline} PT_CHECKPOINT_DIR
ROOT_DIR=.
NORM_NAME=$2
...
OUTPUT_DIR=${ROOT_DIR}/saves/bhyt_exp/lora/sft/com170k/${EXP_NAME}/Rank${LORA_RANK}_DBS${DBS}_GA${GA}_LR${LR}_WD${WD}_MLR${MLR}_MIN_LR${MIN_LR}_WR${WR}

echo "RUN CONFIG: NORM_NAME=${NORM_NAME} LORA_RANK=${LORA_RANK}_${DBS}_${GA}_${LR}_${WD}_${MLR}_${MIN_LR}"

# if [ -d "${OUTPUT_DIR}/checkpoint-9563" ]; then
