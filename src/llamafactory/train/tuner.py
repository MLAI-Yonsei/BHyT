...
from .callbacks import LogCallback, PissaConvertCallback, ReporterCallback, UpdateScalerCallback, LayerMonitoringCallback, LayerMonitoringCallbackForBHyT
...
def _training_function(config: dict[str, Any]) -> None:
    ...
    if (model_args.model_name_or_path.split("_")[-1] == 'pt') and (model_args.model_name_or_path.split("_")[-2].lower() in ['bhyt', 'bhytline']):
        logger.info("Add UpdateScalerCallback for BHyT PT (auto interval)")
        callbacks.append(UpdateScalerCallback())
        callbacks.append(LayerMonitoringCallback())
    elif (model_args.model_name_or_path.split("_")[-1] == 'pt') and not (model_args.model_name_or_path.split("_")[-2].lower() in ['bhyt', 'bhytline']):
        callbacks.append(LayerMonitoringCallback())
    elif model_args.model_name_or_path.split("/")[-3].split("_")[-1].lower() in ['bhyt', 'bhytline']:
        logger.info("Add UpdateScalerCallback for BHyT SFT (auto interval)")
        callbacks.append(UpdateScalerCallback())
        callbacks.append(LayerMonitoringCallback())
    else:
        callbacks.append(LayerMonitoringCallback())
