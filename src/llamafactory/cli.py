...
def main():
    ...
    # Lazy-import wrappers to avoid importing heavy backends (e.g., vllm) unless needed.
    def _cmd_api():
        from .api.app import run_api  # local import to avoid vllm import on non-API commands
        run_api()

    def _cmd_chat():
        from .chat.chat_model import run_chat  # local import
        run_chat()

    def _cmd_eval():
        from .eval.evaluator import run_eval  # local import
        run_eval()

    def _cmd_export():
        from .train.tuner import export_model  # local import
        export_model()

    def _cmd_train():
        from .train.tuner import run_exp  # local import
        run_exp()

    def _cmd_webchat():
        from .webui.interface import run_web_demo  # local import
        run_web_demo()

    def _cmd_webui():
        from .webui.interface import run_web_ui  # local import
        run_web_ui()
    ...
    if command == "train" and (is_env_enabled("FORCE_TORCHRUN") or (get_device_count() > 1 and not use_ray())):
        ...
        # Import here to avoid importing when not needed by other commands
