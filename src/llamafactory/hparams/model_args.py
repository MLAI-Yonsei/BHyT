...
class BaseModelArguments:
    ...
    input_layer_lam: Optional[float] = field(
        default=None,
        metadata={
            "help": "Input layer regularization coefficient in BHyT."
        },
    )
    post_layer_lam: Optional[float] = field(
        default=None,
        metadata={"help": "Post-attention layer regularization coefficient in BHyT."},
    )
    last_layer_lam: Optional[float] = field(
        default=None,
        metadata={"help": "Last layer regularization coefficient in BHyT."},
    )
    var_approx_method: Optional[str] = field(
        default=None,
        metadata={"help": "Variance approximation method for BHyT: 'baseline', 'beta', or 'eta'."},
    )
    use_gamma_var: bool = field(
        default=False,
        metadata={"help": "Include gamma squared mean in BHyT variance approximation."},
    )
    var_approx_exact_layers: int = field(
        default=0,
        metadata={"help": "Number of initial layers to use exact variance instead of approximation (hybrid method). 0=all approx."},
    )
    var_approx_exact_layer_list: Optional[str] = field(
        default=None,
        metadata={"help": "Comma-separated explicit layer indices to use exact variance, e.g. '0,8'. Combined (OR) with var_approx_exact_layers. None=unused."},
    )
    bhyt_kappa: float = field(
        default=10.0,
        metadata={"help": "BHyT kappa parameter controlling tanh operating range. rstd = rsqrt(var) * (1/kappa). Default=10.0 (original)."},
    )
    bhyt_kappa_schedule: Optional[str] = field(
        default=None,
        metadata={"help": "Per-layer kappa schedule. Options: 'decay' (kappa/sqrt(l+1)), 'grow' (kappa*sqrt(l+1)), 'mild_grow' (kappa*(l+1)^0.25). None=uniform."},
    )
    bhyt_layer_range: Optional[str] = field(
        default=None,
        metadata={"help": "Layer range for BHyT, e.g., '0,7' means layers 0-7 use BHyT. None=all layers."},
    )
    fallback_norm_type: Optional[str] = field(
        default=None,
        metadata={"help": "Norm type for layers outside bhyt_layer_range. Default: 'rms'."},
    )
    var_aux_loss_weight: float = field(
        default=0.0,
        metadata={"help": "Weight for variance approximation auxiliary loss. 0.0=disabled."},
    )
    use_shared_scale: bool = field(
        default=False,
        metadata={"help": "Use learnable shared scale parameter after token embedding (init: sqrt(hidden_size))."},
    )
    model_size: Optional[str] = field(
        default=None,
        metadata={"help": "Model size override, e.g., '374M'. Loads architecture config from config/llama_{size}_config.yaml."},
    )
